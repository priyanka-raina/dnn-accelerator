...
proc setup_libraries {} {
    ...
    #   this would include SRAMs, and also standard cells and DesignWare modules that are directly instantiated

    # cache results of analysing the standard cell library for faster synthesis
    set_app_var alib_library_analysis_path ~

	# TODO
    add_library <path to library> -target -link -max wc.db -min bc.db
...
proc setup_design {} {
    ...
    # (* sync_set_reset = "RST_N" *)

    set VERILOG_SRC [list]

    # design files
	# TODO
    lappend VERILOG_SRC <verilog file name> 
    
	# TODO
	set design <top level name>
...
proc config_libraries {} {
    ...
    set_operating_conditions -max WCCOM -min BCCOM -max_library <library name> -min_library <library name>
}

proc config_design {} {
    set clk200_period 4.0

    set clk CLK
    create_clock $clk -period $clk200_period
    set_clock_uncertainty 0.2 $clk
    set_dont_touch_network $clk
    set_ideal_network $clk

    set inputs [remove_from_collection [all_inputs] [list $clk RST_N]]
    set_input_delay 2 -max -clock $clk $inputs
    set_input_delay 0 -min -clock $clk $inputs
    set_input_delay 0.2 -max -clock $clk [get_ports RST_N]
    set_input_delay 0 -min -clock $clk [get_ports RST_N]


    set ref_output [get_port rd_params_pcredit]
    foreach_in_collection output [remove_from_collection [all_outputs] $ref_output] {
        ...
        set_data_check -from $ref_output -to $output -hold [expr $clk200_period - 1]
    }
